...
EOD;
    ...
    protected function startSheet()
    {
        ...
        fwrite($this->sheetFilePointer, '<cols><col min="1" max="100" width="25"/></cols>');
    ...
    public function close()
    {
        ...
        if (!empty($this->externalSheet->getMergeCells())) {
            $start = '<mergeCells count="'.count($this->externalSheet->getMergeCells()).'">';
            $cells = '';
            foreach ($this->externalSheet->getMergeCells() as $mergeRange) {
                $cells .= '<mergeCell ref="'.$mergeRange.'"/>';
            }
            $end = '</mergeCells>';

            fwrite($this->sheetFilePointer, $start.$cells.$end);
        }
